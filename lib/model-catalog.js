 ...
 * The stream half stays the same dead-man fallback — the llm/stream waterfall
 * listener still takes over before dispatch.
 */
export function createClaudeCodeAdapter(settings) {
  ...
  // Advisory exact-model lookup shared by resolveModel and prepareCall (see
  // the resolveModel comment below for the invariants it upholds).
  ...
  return {
    ...
    // DSH 2.0.2 LlmAdapter contract: bind exact model metadata and dispatch to
    // one adapter generation. normalizeModelInfo consumes `.model` verbatim
    // (same shape resolveModel already returns); `.stream` is the never-reached
    // dead-man dispatch. Referencing the outer helpers (not `this`) keeps the
    // methods safe under detached/destructured invocation.
