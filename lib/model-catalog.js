...
//
// Auto-discovery: when `autoDiscoverModels` is on (default), the catalog is
// built from the Claude Agent SDK's own `query.supportedModels()` list (the
// real, current model set) so new model families / versions appear without
// editing the plugin or config. On any discovery failure it falls back to the
// configured `settings.models`. Set `autoDiscoverModels: false` (or provide a
// `settings.models` override) to keep a hand-curated list. contextWindow is
// parsed from the resolvedModel's `[...]` marker, else looked up in a known map,
// else omitted (compaction stays off for that model; the model is still usable).
...
// The effective catalog: prefer an injected async `discover()` returning the SDK
// ModelInfo list, else the configured `settings.models`. Kept cached externally
// by callers (the catalog promise) so discovery runs once.
export async function resolveModelList(settings, discover) {
  if (typeof discover === 'function') {
    try {
      const infos = await discover()
      const list = (infos ?? []).map(modelInfoToEntry).filter(Boolean)
      if (list.length) return list
    } catch {
      // discovery unavailable/failed — fall through to the configured list
    }
  }
  return normalizeModels(settings.models)
}
