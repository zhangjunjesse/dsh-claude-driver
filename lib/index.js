...
const DEFAULTS = {
  ...
  // Claude Code performs (SDK does not stream those otherwise), so the DSH
  // surface is never silent while the agent explores. Rides the existing text
  // block (index 0); disable with showToolProgress:false.
...
// tools/pre-execute pipeline; Claude built-in tools keep the SDK's default
// permission handling (permissionMode). Exported for the mock card tests.
export function buildToolBridge(ctx, options, settings, controller) {
  ...
  // MCP tools surface as `mcp__<server>__<tool>`; allow ours (DSH pipeline
  // handles approval), fall back to SDK defaults for everything else.
  const canUseTool = async (toolName, input) => {
    const base = dsdNames.has(toolName)
      ? toolName
      : [...dsdNames].find((name) => toolName.endsWith(`__${name}`))
    // Bridged DSH tools render their own native card and go through DSH's own
    // tools/pre-execute → approval pipeline, so allow here (double-prompt
    // otherwise). Claude built-in tools (Bash/Edit/Read/Agent/…) are explicitly
    // allowed too: returning null is fail-closed in the SDK (no control_response
    // → the tool blocks forever, no deadline). A proper DSH-approval bridge for
    // built-ins is a tracked follow-up.
    if (base === undefined) return { behavior: 'allow', updatedInput: input }
  ...
  return { mcpServers: [server], canUseTool, rendersCard, handlers }
}
  ...
  try {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        ...
        if (bridge) {
          sdkOptions.mcpServers = bridge.mcpServers
          sdkOptions.canUseTool = bridge.canUseTool
        }
