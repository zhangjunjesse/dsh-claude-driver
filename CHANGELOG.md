...
### Fixed
- **预热进程的空闲 TTL 会杀死正在干活的回合**（0.6.0 引入的严重回归。用户现场：
  回合转圈 50 分钟不出结果、发的新消息看不见、重启后消息才冒出来）。
  - **根因**：TTL 定时器在 `createPrewarmEntry` 时按「创建时刻 + 15 分钟」武装，
    而领养走的 `takePrewarm()` 只把条目从池子里摘走、**从不解除这个定时器**。
    于是进程被领养、正作为某个活着回合的传输层用着时，旧的空闲死线照样开火，
    `dispose()` 关掉输入流并 `controller.abort()`，把 CLI 拦腰砍断。
  - **现场日志**（`dsh-2026-09-17.log`）逐秒吻合：
    `09:39:56 prewarmed …386b8a27` → `09:51:58 adopted …386b8a27` →
    `09:54:56 disposed: idle TTL expired`（= 创建时刻 + 15 分钟，分秒不差）。
  - **修复①（根因）**：`takePrewarm()` 在条目离开池子时一律 `clearTimeout`。
    这是唯一出池口（领养与 `disposePrewarm` 都经过它），一处即全覆盖。TTL 的
    语义本来就是「**未被领养**的进程闲置了多久」，离池即应失效。
  - **修复②（纵深防御）**：`entry.dispose()` 现在会唤醒正在读该条目的消费者
    （置 `ended` + `error` 并 flush `waiters`）。此前领养只接了 run→entry 的
    中止链、**没有反向**：外部杀掉 entry 时主循环仍在 `await` 一个永远不会到来
    的消息 —— 而 SDK 的 iterator 在 abort 后并不可靠终止（同一个坑使
    `iterator.return()` 在开放输入下永久挂起，见 [0.6.0]），于是「报错」退化成
    「永久挂起」。现在任何来源的传输层死亡都在毫秒级浮现为失败；若此时尚无
    输出，既有的 resume 重试逻辑会自动冷启动重跑，用户仍能拿到回答。
  - **测试**：`test-prewarm.mjs` 新增 case 9（9a 定点断言「出池即解除武装」、
    9b 端到端：领养的回合活过旧死线后仍由**被领养会话**作答，而不是静默降级成
    冷重试）与 case 10（活条目被外部杀死后必须快速失败并冷重试，带耗时上限）。
    两条均做过**变异验证**：撤掉修复① 时 9a/9b 变红（9b 实测 `text="recovered"`、
    fresh spawn = 2，正是「被杀后降级」的指纹）；撤掉修复② 时 case 10 变红
    （`took 3008ms`；真实 SDK 下则是永久挂起，只会更糟）。

### Notes（DSH 会话层的放大器，非本插件可修）
本次事故之所以难懂，是因为驱动的一次挂起被 DSH 会话层放大了三重。排查时已定位
到具体位置，记录备查（均在 `@deepseek-ai/*` 包内）：
- **回合运行中发出的消息不进聊天记录**：`dsh-client-ui-chat` 把
  `placement:'queued'` 的条目显式排除在正文之外，只出现在输入框上方的「队列」
  条里 —— 所以体感是「我的消息不显示」。默认 Enter 行为是 `queue` 而非 `steer`。
- **agent 循环没有任何回合级超时/停滞检测**：`dsh-agent-loop` 内找不到 turn/step
  级 watchdog；唯一的 5 分钟空闲看门狗在 DeepSeek adapter 内部，且工具执行期间
  不计时。驱动一挂起，`running` 位永不清零，「深度求索中」无限转。
- **重启后队列里的消息不会自动投递**：崩溃修复只补齐 `turn/end`，agent 以 `idle`
  发布，`inbox` 里的消息要等用户再发一条或手动 steer 才被认领。
  （同理：回合中途按「停止」或传输层报错，也会把已排队的消息滞留在 inbox。）
