...
// The value handed to query({prompt}): the plain prompt string when the plan
// carries no image, otherwise a one-shot AsyncIterable<SDKUserMessage> whose
// single user message carries the interleaved text/image content blocks. When
// EVERY image degrades to a placeholder the string path is kept (identical to
// plan.prompt by construction).
export async function buildSdkPrompt(ctx, plan, signal) {
  if (!segmentsHaveImage(plan?.segments)) return plan.prompt
  let attachments
  try {
  ...
  try {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        ...
        const promptInput = await buildSdkPrompt(ctx, plan, controller.signal)
        const q = runQuery({
          prompt: promptInput,
          options: sdkOptions,
        })
